...
public:
    ...
    float ellipticCone(glm::f32vec3 pos, glm::f32vec3 center, float h, float R, float a, float b);
    float truncatedPyramid(glm::f32vec3 pos, glm::f32vec3 cen, glm::f32vec2 botRec, glm::f32vec2 topRec, float H);
    float squarePyramid(glm::f32vec3 pos, glm::f32vec3 center, float a, float h);
    float truncPyramid(glm::f32vec3 pos, glm::f32vec3 center, float a, float b, float h);
    float octahedron(glm::f32vec3 pos, glm::f32vec3 cen, float a, float h);
    float tetrahedron(glm::f32vec3 pos, glm::f32vec3 cen, float a);

    float box(glm::f32vec3 pos, glm::f32vec3 center, float w, float h, float d);
    float heart3D(glm::f32vec3 pos, glm::f32vec3 center);
    float BarthDecic(glm::f32vec3 pos, glm::f32vec3 center, float c);
    float flower(glm::f32vec3 pos, glm::f32vec3 center);


    std::vector<float> getShelledObj(std::vector<float> *frepObj, float offset, cut cutM, glm::f32vec3 *cutP);
    //*****************************************************
    ...
    std::vector<float> getFRep3D(glm::f32vec3 cent, glm::f32vec2 a, glm::f32vec2 b, float c,
                                 std::function<float(glm::f32vec3, glm::f32vec3, glm::f32vec2, glm::f32vec2, float)> fun);
...
private:
    float scaleToNewRange(float v, float newMin=0, float newMax = 0, float oldMin = 0, float oldMax = 128);
    ...
    inline int index3d( int x, int y, int z ) { return z*resolutionX*resolutionY + y*resolutionX + x; }
