...
public:
    ...
    void solveEikonalParallel_CPU( std::vector<float> *srcField, float frep_zero_bot, float frep_zero_up );
...
private:
    ...
    float godunovSolver(int curInd, float speedFun , float h = 1.0f);
    std::vector<float> getMinValues(int curInd);
    std::vector<int> getNeightboursIdexes(int curInd);
    ...
    inline float calculateINF() { return std::max(res_x, std::max(res_y, res_z)) + 1; }
    ...
    inline void normField(){ std::transform( dField.begin(), dField.end(), dField.begin(),
                                             std::bind2nd(std::multiplies<float>(), 1.0f/Inf)); }
