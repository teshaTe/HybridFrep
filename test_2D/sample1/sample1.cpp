...
int main(int argc, char** argv)
{
    ...
    function_rep::stepFuncParams st_function;
    st_function.sigmoid_slope    = 0.00001;
    st_function.tangent_slope    = 100000.0;
    st_function.algebraic_slope  = 0.0001;
    st_function.guderanian_slope = 1000.0;
    int step_func = HYPERBOLIC_SIGMOID;

    hfrep.calculateHFRep( set_geometry("heart"), geo, step_func, st_function, nullptr, nullptr );
    std::vector<double> hfrep2  = hfrep.getHFRepVec();
    std::vector<double> sddt2   = hfrep.getSignedDDT();

    uField.clear();
    drawField.draw_rgb_isolines( &uField, &hfrep2, 512, 512, thres_vis_hfrep, "hfrep2", true );
    uField.clear();
    drawField.draw_rgb_isolines( &uField, &sddt2, 512, 512, thres_vis_ddt, "sddt2", true );
