...
inline hfrep2D::Point2Df convertToUV( hfrep2D::Point2Df val ) { return hfrep2D::Point2Df( val.dx/512.0f, val.dy/512.0f ); }

float find_Y_coordinate( hfrep2D::Point2Df cent1, hfrep2D::Point2Df cent2, float R1, float R2 )
{
    float X, Y;
...
int main(int argc, char** argv)
{
    ...
    std::vector<float> circle0 = frep.getFRep2D( hfrep2D::Point2Df( 250.0f, 150.0f ), 60.0f, fun1 );
    std::vector<float> circle1 = frep.getFRep2D( hfrep2D::Point2Df( 250.0f, 230.0f ), 90.0f, fun1 );
    ...
    for (int y = 0; y < 512 ; y++)
    {
        for (int x = 0; x < 512; x++)
        {
            hfrep2D::Point2Df grF1 = hfrep2D::Point2Df( sddt_circle0[x+1+y*512]   - sddt_circle0[x+y*512],
                                        sddt_circle0[x+(y+1)*512] - sddt_circle0[x+y*512] );
            hfrep2D::Point2Df grF2 = hfrep2D::Point2Df( sddt_rec[x+1+y*512]   - sddt_rec[x+y*512],
    ...
    hfrep2D::Point2Df cent1 = convertToUV( hfrep2D::Point2Df( 250.0f, 150.0f ) );
    hfrep2D::Point2Df cent2 = convertToUV( hfrep2D::Point2Df( 250.0f, 230.0f ) );
    ...
    for (int y = 0; y < 512; y++)
    {
        for (int x = 0; x < 512; x++)
        {
            ...
            hfrep2D::Point2Df grF1 = hfrep2D::Point2Df( -(u - cent1.dx)/
                                   std::sqrt((u - cent1.dx)*(u - cent1.dx) + (v - cent1.dy)*(v - cent1.dy)),
                                                    -(v - cent1.dy)/
                                   std::sqrt((u - cent1.dx)*(u - cent1.dx) + (v - cent1.dy)*(v - cent1.dy)));

            hfrep2D::Point2Df grF2 = hfrep2D::Point2Df( -(u - cent2.dx)/
