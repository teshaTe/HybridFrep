...
distance_matrix::distance_matrix(const matrix_t& x, int num_threads) : x_(x)
{
    #ifdef _OPENMP
...
distance_matrix::distance_matrix(const matrix_t& x, const vector_t& w, int num_threads) : x_(x), w_(w)
{
    #ifdef _OPENMP
        if(num_threads) omp_set_num_threads(num_threads);
    #endif
