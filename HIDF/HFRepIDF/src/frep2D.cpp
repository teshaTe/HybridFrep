...
float FRepObj2D::i_shape(glm::vec2 pos, glm::vec2 center)
{
    float box1 = rectangle(pos, glm::vec2(center.x, center.y + scaleToNewRange(30)/scale), 50, 90);
    float box2 = rectangle(pos, glm::vec2(center.x, center.y + scaleToNewRange(30)/scale), 20, 60);
    float i_base = subtract_function(box1, box2);

    float circle1 = circle(pos, glm::vec2(center.x, center.y - scaleToNewRange(120)/scale), 50);
    float circle2 = circle(pos, glm::vec2(center.x - scaleToNewRange(10)/scale, center.y - scaleToNewRange(150)/scale), 20);
    //float circle3 = circle(pos, glm::vec2(center.x + scaleToNewRange(10)/scale, center.y - scaleToNewRange(110)/scale), 10);

    //float i_top0 = subtract_function(circle1, circle2);
    float i_top  = subtract_function(circle1, circle2);
    float result = union_function(i_base, i_top);
    return result;
}

float frep::FRepObj2D::c_shape(glm::vec2 pos, glm::vec2 center)
{
    glm::vec2 offset(scaleToNewRange(20.0)/scale, 0);
    float in_base = circle(pos, glm::vec2(center.x+scaleToNewRange(30)/scale, center.y-scaleToNewRange(30)/scale) -offset, 120);
    float circle1 = circle(pos, glm::vec2(center.x+scaleToNewRange(30)/scale, center.y-scaleToNewRange(130)/scale)-offset, 80);
    float circle2 = circle(pos, glm::vec2(center.x-scaleToNewRange(20)/scale, center.y+scaleToNewRange(10)/scale) -offset, 30);
    float circle3 = circle(pos, glm::vec2(center.x+scaleToNewRange(80)/scale, center.y+scaleToNewRange(10)/scale) -offset, 30);

    float hals     = union_function(circle2, circle3);
    float c_shape0 = subtract_function(in_base, circle1);
    float result   = subtract_function(c_shape0, hals);
    return result;
}
...
float FRepObj2D::trebleClef(glm::vec2 pos, glm::vec2 center)
{
    ...
    float rec_helper1 = rectangle(pos, glm::vec2(center.x, center.y - scaleToNewRange(87)/scale), scaleToNewRange(100), 50);
